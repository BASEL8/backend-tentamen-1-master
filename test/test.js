...
describe("/shopping", function() {
  ...
  it("Handles GET / when list is empty", function() {
    return expectReq("GET", 200).then((res) => {
  ...
  it("Handles GET / when list is populated", function() {
    ...
    return Promise.all(data.map((x) => expectReq("POST", 201, x)))
      ...
      .then((res) => {
        ...
        let ok = data.every((x) =>
          list.find((y) => y.name === x.name && y.amount === x.amount)
        );
        assert.ok(ok);
      });
  });
  ...
  it("Handles pagination for GET /", function() {
    ...
    return Promise.all(data.map((x) => expectReq("POST", 201, x)))
      ...
      .then((res) => {
        ...
        assert.ok(ok);
        return axios
          .get("http://localhost:" + PORT + "/shopping?page=2")
          .then((res2) => {
      ...
      .then((res3) => {
        ...
        return axios.get(
          "http://localhost:" + PORT + "/shopping?page=1&size=100"
        );
      })
      .then((res4) => {
  ...
  it("Handles DELETE /:name", function() {
    ...
    return Promise.all([expectReq("POST", 201, o), expectReq("POST", 201, o2)])
